---
description: Validates specification documents for goal coverage, internal consistency, codebase grounding, technical feasibility, and spec quality — GPT perspective
mode: subagent
...
> Verify the following claims from the spec: (1) the file at [path from spec] has a [method name] method, (2) [ClassName] in [path from spec] has an abstract execute() method, (3) [directory from spec] exists. For each, report whether the claim is accurate and provide the actual file:line if it exists.
...
- A checkpoint exists after foundation beads (typically the second child)
- A checkpoint exists after each major module group
- A checkpoint exists as the **final child of the epic** — this is mandatory, no exceptions
...
- The Bead and Sub-Item(s) columns reference real bead IDs and sub-item numbers
...
- Bead depends on another bead that creates a dependency cycle (verify with `bd dep cycles`)
...
- Coverage summary has missing rows — acceptance criteria exist in requirements.md but have no row in the table
...
- Incomplete cross-reference (requirement exists but epic description coverage summary misses it)
...
- Checkpoint bead missing after foundation beads or after a major module group
...
### Step 2: Read the Spec Files and Beads

Read the spec files and implementation epic:
...
3. Implementation epic — run `bd show <epic-id>` for the epic description (including the Requirements Coverage Summary table), then `bd children <epic-id> --json` to list all child beads, then `bd show <child-id>` for each child's sub-items, checkpoint placement, and PBT sub-items
...
- Walk each bead backward to its requirement
- Check the coverage summary table for completeness — every criterion must have a row
...
Check implementation epic structural requirements:
...
- Final checkpoint bead verifies PBT tests specifically?
- Coverage summary table in the epic description has a row for every acceptance criterion?
...
- **Foundation checkpoint bead exists**: YES | NO
- **Module checkpoint beads exist**: [X found for Y module groups]
- **Final checkpoint bead exists**: YES | NO
- **Coverage summary rows (epic description)**: [X/Y criteria covered]
